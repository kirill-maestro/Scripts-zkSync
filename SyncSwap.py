...
def syncswap_swap(private_key, amount, from_token, to_token):
    ...
    def get_pool(from_token: str, to_token: str):
            contract_get_pool = get_contract(SYNCSWAP_CONTRACT["classic_pool"], w3, SYNCSWAP_CLASSIC_POOL_ABI)
    ...
    def get_min_amount_out(pool_address: str, token_address: str, amount: int, slippage: float):
        pool_contract = get_contract(pool_address, w3, SYNCSWAP_CLASSIC_POOL__DATA_ABI)
    ...
    def swap(
        ...
        if pool_address != ZERO_ADDRESS:
            if from_token == "ETH":
                ...
                approve(amount_wei, token_address, Web3.to_checksum_address(SYNCSWAP_CONTRACT["router"]), account, w3)
